...
- (void)addActionButtonPlaceholderKey:(UILongPressGestureRecognizer *)sender
{
    ...
    {
        NSArray *placeholders =
            @[ GetSocialActionType.Custom, GetSocialActionType.OpenActivity, GetSocialActionType.OpenProfile, GetSocialActionType.OpenInvites, GetSocialActionType.OpenUrl ];
        UISimpleAlertViewController *alert = [[UISimpleAlertViewController alloc] initWithTitle:@"Add Placeholders"
                                                                                        message:@"Choose the placeholder to add"
                                                                              cancelButtonTitle:@"Cancel"
                                                                              otherButtonTitles:placeholders];
...
{
    [self toggleRecipient:GetSocialNotificationReceiversPlaceholders.Friends withButton:sender];
}

- (IBAction)toggleReferrer:(id)sender
{
    [self toggleRecipient:GetSocialNotificationReceiversPlaceholders.Referrer withButton:sender];
}

- (IBAction)toggleReferredUsers:(id)sender
{
    [self toggleRecipient:GetSocialNotificationReceiversPlaceholders.ReferredUsers withButton:sender];
}
...
- (IBAction)sendNotification:(id)sender
{
    ...
    if (self.templateName.text.length == 0 && self.customText.text.length == 0 && [selectedAction isEqual:GetSocialActionType.AddFriend])
    {
        ...
        [action setDataValue:[GetSocial.currentUser userId] forKey: GetSocialActionDataKey.AddFriend_UserId];
        [action setDataValue:[GetSocial.currentUser displayName] forKey: @"user_name"];
        [content setAction:action];
        content.actionButtons = [self createActionButtons];

        GetSocialSendNotificationTarget * target = [GetSocialSendNotificationTarget usersWithIds: [GetSocialUserIdList usersWithIds:[self createUserIds]]];
...
- (NSDictionary<NSString*, NSString*> *)actions
{
    return @{
        ...
        @"Default" : GetSocialActionType.Custom,
