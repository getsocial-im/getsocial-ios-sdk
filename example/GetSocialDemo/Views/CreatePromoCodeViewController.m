...
{
    sender.enabled = NO;
    GetSocialPromoCodeContent *promoCodeContent =
        self.promoCode.text.length > 0 ? [GetSocialPromoCodeContent withCode:self.promoCode.text] : [GetSocialPromoCodeContent withRandomCode];

    promoCodeContent.properties = [self createCustomData];
    [promoCodeContent setTimeLimitWithStartDate:self.startDate endDate:self.endDate];
    promoCodeContent.maxClaims = self.maxClaimCount.text.intValue;

    [GetSocialPromoCodes createWithContent:promoCodeContent
