...
- (IBAction)addTag:(UIGestureRecognizer *)sender
{
    ...
    {
        NSArray *tags = @[
            GetSocialInviteContentPlaceholder.AppInviteUrl, GetSocialInviteContentPlaceholder.PromoCode,
            GetSocialInviteContentPlaceholder.UserName
        ];
...
- (IBAction)onOpenSmartInvite:(id)sender
{
    ...
    GetSocialInviteContent *mutableInviteContent = [GetSocialInviteContent new];
    ...
    {
        mutableInviteContent.mediaAttachment = [GetSocialMediaAttachment withImageUrl:self.imageUrlField.text];
    }
    else if (self.customImage.image)
    {
        mutableInviteContent.mediaAttachment = [GetSocialMediaAttachment withImage:self.customImage.image];
    }
    else if (self.customVideoContent)
    {
        mutableInviteContent.mediaAttachment = [GetSocialMediaAttachment withVideo:self.customVideoContent];
    }

    GetSocialUIInvitesView *invitesView = [GetSocialInvitesUI createView];
    [mutableInviteContent setLinkParams:[self addLandingPageCustomization]];
    [invitesView setCustomInviteContent:mutableInviteContent];

    if ([self.textfield.text rangeOfString:GetSocialInviteContentPlaceholder.AppInviteUrl].location == NSNotFound)
    {
...
- (NSMutableDictionary *)addLandingPageCustomization
{
    ...
    {
        linkParams[GetSocialLinkParamsPlaceholder.CustomTitle] = self.landingPageTitle.text;
    }
    // add custom description
    if (self.landingPageDescription.text.length > 0)
    {
        linkParams[GetSocialLinkParamsPlaceholder.CustomDescription] = self.landingPageDescription.text;
    }
    // add custom image url
    if (self.landingPageImageUrl.text.length > 0)
    {
        linkParams[GetSocialLinkParamsPlaceholder.CustomImage] = self.landingPageImageUrl.text;
    }
    // add custom video url
    if (self.landingPageVideoUrl.text.length > 0)
    {
        linkParams[GetSocialLinkParamsPlaceholder.CustomYouTubeVideo] = self.landingPageVideoUrl.text;
    }
    // add custom image
    if (self.landingPageImage.image != nil)
    {
        linkParams[GetSocialLinkParamsPlaceholder.CustomImage] = self.landingPageImage.image;
    }
