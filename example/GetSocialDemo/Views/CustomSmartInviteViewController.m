...
- (IBAction)addTag:(UIGestureRecognizer *)sender
{
    ...
    {
        NSArray *tags = @[
            GetSocialInviteContentPlaceholders.inviteUrl, GetSocialInviteContentPlaceholders.promoCode,
            GetSocialInviteContentPlaceholders.userName
        ];
...
- (IBAction)onOpenSmartInvite:(id)sender
{
    ...
    if ([self.textfield.text rangeOfString:GetSocialInviteContentPlaceholders.inviteUrl].location == NSNotFound)
    {
...
- (NSMutableDictionary *)addLandingPageCustomization
{
    ...
    {
        linkParams[GetSocialLinkParams.customTitle] = self.landingPageTitle.text;
    }
    // add custom description
    if (self.landingPageDescription.text.length > 0)
    {
        linkParams[GetSocialLinkParams.customDescription] = self.landingPageDescription.text;
    }
    // add custom image url
    if (self.landingPageImageUrl.text.length > 0)
    {
        linkParams[GetSocialLinkParams.customImage] = self.landingPageImageUrl.text;
    }
    // add custom video url
    if (self.landingPageVideoUrl.text.length > 0)
    {
        linkParams[GetSocialLinkParams.customYouTubeVideo] = self.landingPageVideoUrl.text;
    }
    // add custom image
    if (self.landingPageImage.image != nil)
    {
        linkParams[GetSocialLinkParams.customImage] = self.landingPageImage.image;
    }
