...
- (void)viewDidLoad
{
    ...
    [self.actionTypePicker selectRow:3 inComponent:0 animated:YES];
}
...
- (void) viewWillAppear:(BOOL)animated {
    if (self.activityToUpdate != nil) {
        ...
        if (button != nil) {
            ...
            NSDictionary* allActions = [self actions];
            for (int i = 0; i < allActions.allValues.count; i++) {
                if ([allActions.allValues[i] isEqualToString:self.activityToUpdate.button.action.type]) {
                    [self.actionTypePicker selectRow:i inComponent:0 animated:YES];
                }
            }
        }
...
- (NSDictionary<NSString*, NSArray<NSString *> *> *)actionKeyPlaceholders
{
    return @{
        ...
        GetSocialActionType.openActivity : @[
            ...
            GetSocialActionDataKey.openActivity_CommentId, GetSocialActionDataKey.openActivity_UserId, GetSocialActionDataKey.openActivity_GroupId
        ]
    };
}
...
- (void) callUpdateActivity
{
    ...
    GetSocialUIActivityDetailsView *view = [GetSocialUIActivityDetailsView  viewForActivityId:self.activityToUpdate.activityId];
...
- (IBAction)postActivity:(id)sender
{
    ...
    GetSocialActivitiesQuery* query = nil;
    if (self.postTarget.type == GetSocialCommunitiesEntityTypeUser) {
        query = [GetSocialActivitiesQuery timeline];
    }
    if (self.postTarget.type == GetSocialCommunitiesEntityTypeTopic) {
        query = [GetSocialActivitiesQuery inTopicWithId:self.postTarget.targetId];
    }
