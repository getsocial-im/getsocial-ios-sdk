...
- (BOOL)handleAction:(GetSocialAction *)action
{
    ...
    if ([action.type isEqualToString:GetSocialActionType.Custom])
    {
...
- (void)handleAction:(NSString *)action withPost:(GetSocialActivity * __unused)post
{
...
{
    return ^void(NSString *action, GetSocialActivity *post) {
...
- (void)updateFriendsCount
{
    ...
    GetSocialFriendsQuery* query = [GetSocialFriendsQuery ofUserWithId:[GetSocialUserId userWithId: GetSocial.currentUser.userId]];
...
- (void)loadMenu
{
    if (!self.menu)
    {
        ...
        [self.activitiesMenu
            addSubmenu:[MenuItem actionableMenuItemWithTitle:@"Timeline with handlers"
                                                      action:^{
                                                        GetSocialActivitiesQuery* query = [GetSocialActivitiesQuery timeline];
                                                          GetSocialUIActivityFeedView *activityFeedView = [GetSocialFeedsUI createActivityFeedView:query];
                                                          ...
                                                          [activityFeedView setMentionClickHandler:^(NSString* mention) {
                                                              ...
                                                              GetSocialUserIdList* userIdList = [GetSocialUserIdList usersWithIds:@[mention]];
                                                          ...
                                                          [activityFeedView setTagClickHandler:^(NSString *tagName) {
                                                              NSLog(@"Tag clicked");
//                                                              GetSocialUIActivityFeedView *tagFeedView = [GetSocialFeedsUI createGlobalActivityFeedView];
        ...
        [self.activitiesMenu addSubmenu:[MenuItem actionableMenuItemWithTitle:@"Topic Activity Feed (DemoFeed)"
            ...
            GetSocialActivitiesQuery* query = [GetSocialActivitiesQuery inTopicWithId:@"DemoFeed"];
                                                                           GetSocialUIActivityFeedView *activityFeedView =
                                                                               [GetSocialFeedsUI createActivityFeedView:query];
                                                                           [activityFeedView setActionHandler:[self defaultActionHandler]];
                                                                           [activityFeedView show];
                                                                       }]];

        [self.activitiesMenu addSubmenu:[MenuItem actionableMenuItemWithTitle:@"My Activity Feed (not available in phase 1)"
                                                                       action:^{
...
- (void)didClickOnUser:(GetSocialUser *)user
{
    ...
    [GetSocialCommunities areFriendsWithIds:[GetSocialUserIdList usersWithIds:@[user.userId]]
    ...
    [alertViewController showWithDismissHandler:^(NSInteger selectedIndex, NSString *selectedTitle, BOOL didCancel) {
        if (!didCancel)
        {
            switch (selectedIndex)
            {
                ...
                case 1:
                    [GetSocialCommunities removeFriendsWithIds:[GetSocialUserIdList usersWithIds:@[user.userId]]
    ...
    [alertViewController showWithDismissHandler:^(NSInteger selectedIndex, NSString *selectedTitle, BOOL didCancel) {
        if (!didCancel)
        {
            switch (selectedIndex)
            {
                ...
                case 1:
                    [GetSocialCommunities addFriendsWithIds:[GetSocialUserIdList usersWithIds:@[user.userId]]
                        success:^(NSInteger friendsCount) {
...
- (void)showNewFriend
{
    [GetSocialCommunities usersWithIds:[GetSocialUserIdList usersWithIds:@[self.userIdToShow]]
...
{
    [[GetSocialInvitesUI createView] show];
}
...
{
    GetSocialUINotificationCenterView *ncView = [GetSocialNotificationsUI createNotificationCenterView];
    [ncView setClickHandler:^BOOL(GetSocialNotification *notification) {
        return [self handleAction:notification.notificationAction];
    }];
    [ncView setActionButtonHandler:^BOOL(GetSocialNotification *notification, GetSocialNotificationButton *actionButton) {
...
{
    GetSocialUIInvitesView *invitesView = [GetSocialInvitesUI createView];
    ...
    inviteContent.text = [NSString stringWithFormat:@"Use my Promo Code to get a personal discount %@ . %@",
                                                    GetSocialInviteContentPlaceholder.PromoCode, GetSocialInviteContentPlaceholder.AppInviteUrl];
    [inviteContent setLinkParams:@{GetSocialLinkParamsPlaceholder.PromoCode : code}];
...
- (BOOL)changeLanguage:(NSString *)language
{
    ...
    self.languageMenu.detail = [NSString stringWithFormat:@"Current language: %@", nil];//[GetSocialConstants allLanguageCodes][language]];
