...
- (void)loadMenu
{
    if (!self.menu)
    {
		...
		[self.userAuthenticationMenu addSubmenu:[MenuItem actionableMenuItemWithTitle:@"Increment Property"
																			   action:^{
			[self incrementProperty];
		}]];

		[self.userAuthenticationMenu addSubmenu:[MenuItem actionableMenuItemWithTitle:@"Decrement Property"
																			   action:^{
			[self decrementProperty];
		}]];
...
{
	[self showActivityIndicatorView];
	[GetSocial.currentUser refreshWithSuccess:^{
		[self hideActivityIndicatorView];
		[self showAlertWithText:@"User refreshed"];
	} failure:^(NSError * error) {
		[self hideActivityIndicatorView];
		GSLogError(YES, NO, @"Failed to refresh user, error: %@", error.localizedDescription);
	}];
}

- (void)incrementProperty
{
	UISimpleAlertViewController *alert = [[UISimpleAlertViewController alloc] initWithTitle:@"Increment Property"
																					message:@"Enter key and value"
																		  cancelButtonTitle:@"Cancel"
																		  otherButtonTitles:@[ @"Ok" ]];

	[alert addTextFieldWithPlaceholder:@"Key" defaultText:nil isSecure:NO];
	[alert addNumericTextField];
	...
	[alert showWithDismissHandler:^(NSInteger selectedIndex, NSString *selectedTitle, BOOL didCancel) {
		if (!didCancel)
		{
			...
			double value = [alert valueOfNumericTextFieldAtIndex: 1];
			GetSocialUserUpdate* userUpdate = [GetSocialUserUpdate new];
			[userUpdate increasePublicPropertyWithKey:key by:value];
			[GetSocial.currentUser updateDetailsWith: userUpdate
											 success:^{
				[self hideActivityIndicatorView];
				[[NSNotificationCenter defaultCenter] postNotificationName:UserWasUpdatedNotification object:nil];
...
{
	UISimpleAlertViewController *alert = [[UISimpleAlertViewController alloc] initWithTitle:@"Decrement Property"
																					message:@"Enter key and value"
																		  cancelButtonTitle:@"Cancel"
																		  otherButtonTitles:@[ @"Ok" ]];
	...
	[alert showWithDismissHandler:^(NSInteger selectedIndex, NSString *selectedTitle, BOOL didCancel) {
		if (!didCancel)
		{
			...
			double value = [alert valueOfNumericTextFieldAtIndex: 1];
			GetSocialUserUpdate* userUpdate = [GetSocialUserUpdate new];
			[userUpdate decreasePublicPropertyWithKey:key by:value];
			[GetSocial.currentUser updateDetailsWith: userUpdate
											 success:^{
				[self hideActivityIndicatorView];
				[[NSNotificationCenter defaultCenter] postNotificationName:UserWasUpdatedNotification object:nil];
